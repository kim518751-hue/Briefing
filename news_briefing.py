...
CATEGORIES = {
    "국내은행 글로벌·디지털": [
        # 국가별 (국내은행 전반)
        "은행 베트남 진출", "은행 인도네시아 진출", "은행 필리핀 진출", "은행 캄보디아",
        "은행 방글라데시", "은행 중국 법인", "은행 유럽 진출", "은행 일본 진출", "은행 미국 진출",
        # 은행 × 국가 (주요 조합)
        "신한은행 베트남", "신한은행 인도네시아", "신한은행 일본", "신한은행 우즈베키스탄",
        "우리은행 베트남", "우리은행 인도네시아", "우리은행 캄보디아", "우리은행 방글라데시",
        "하나은행 인도네시아", "하나은행 중국", "KB국민은행 캄보디아", "KB국민은행 인도네시아",
        "기업은행 중국", "기업은행 미국", "농협은행 미국",
        # 은행별 글로벌 일반
        "신한은행 글로벌", "우리은행 글로벌", "하나은행 글로벌", "KB국민은행 글로벌",
        "농협은행 해외", "기업은행 글로벌", "카카오뱅크 해외", "토스뱅크 글로벌",
        # 우산 키워드
        "국내은행 해외진출", "은행 해외법인", "은행 글로벌 전략", "시중은행 동남아",
        "은행 디지털 전환", "은행 해외 디지털",
    ],
    "국제정세·금융": [
...
HOBBY_CATEGORIES = {
    "게임": [
        # PS5 진영
        "PS5", "플레이스테이션5", "PS5 독점작", "PS5 신작", "소니 플레이스테이션",
        # 닌텐도 스위치2 진영
        "닌텐도 스위치2", "스위치2", "Nintendo Switch 2", "닌텐도 신작", "닌텐도 독점",
        # Xbox 진영
        "Xbox", "엑스박스", "Xbox 게임패스", "마이크로소프트 게임",
        # 콘솔 공통
        "콘솔 게임 신작", "콘솔 독점작", "차세대 콘솔", "콘솔 게임 출시", "게임 신작 출시",
    ],
    "피규어·컬렉터블": [
        # 일본 애니 IP
        ...
        "귀멸의칼날 피규어",
        # 건담
        "건담 프라모델", "건프라 신제품", "건담 피규어", "건담 메탈빌드",
        # 핫토이 12인치 / 마블 / DC
        "핫토이 신제품", "핫토이 12인치", "핫토이 마블", "핫토이 DC", "핫토이 예약",
        "마블 핫토이", "마블 레전드", "마블 피규어", "DC 피규어", "DC 핫토이",
        "12인치 피규어", "커스텀 피규어",
        # 반다이 초합금 / 로봇
        "반다이 초합금", "초합금혼", "소울오브초합금", "메탈빌드", "초합금 로봇",
        "프라모델 신제품",
    ],
}
...
def collect(categories=None, recent_days=None):
    ...
    for cat, keywords in categories.items():
        ...
        boost = (cat == "국제정세·금융")   # 선호 매체 우선은 국제정세에만 적용
        bucket.sort(key=lambda x: ((_pref(x) if boost else False), x["pub_sort"]),
                    reverse=True)
        cap = CATEGORY_CAPS.get(cat, MAX_PER_CATEGORY)
        grouped[cat] = bucket[:cap]
